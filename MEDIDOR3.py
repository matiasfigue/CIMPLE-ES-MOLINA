...
if os.path.exists("LOG-OK-M3.txt") and os.path.exists("LOG-OK-EXPORT-M3.csv") and os.path.exists("LOG-BKP-M3.txt") and os.path.exists("LOG-BKP-EXPORT-M3.csv"):
    print("estan creados..")
else:
    create_log = open("LOG-OK-M3.txt", "w")
    create_csv = open("LOG-OK-EXPORT-M3.csv", "w")
    create_bkp = open("LOG-BKP-M3.txt", "w")
    create_csv_bkp = open("LOG-BKP-EXPORT-M3.csv", "w")


if respuesta == 0:  # SI RESPONDE EL PING GUARDA LA DATA EN UN ARCHIVO COMO OK Y ENVIA UN PAQUETE JSON
    create_log = open("LOG-OK-M3.txt", "a")
    create_csv = open("LOG-OK-EXPORT-M3.csv", "a")
...
# LEE TXT Y AGREGA A CSV
    ...
    data_cvs = open("LOG-OK-M3.txt")
    s = [reg_lost]
    lineas = data_cvs.readlines()
    create_cvs = csv.writer(open("LOG-OK-EXPORT-M3.csv", "a+"), delimiter=';', quoting=csv.QUOTE_ALL)
#ENVIANDO JSON A TECNO APLICA
    ...
    print("LOG-OK-M3 GUARDADO...")
# SIN RESPUESTA DEL HOST
else:
    create_bkp = open("LOG-BKP-M3.txt", "a")
    reg_lost = str(fecha) + ";" + str(hora) + ";" + str(device1) + ";" + str(estado1) + ";" + str(vrs1) + ";" +\
               str(vst1) +";"+ str(vtr1) +";"+ str(ir1) + ";" + str(is1) +";"+ str(it1)+ ";" + str(p_avg1)+ ";"+ str(pf_avg)+ ";"+\
               str(demand_active)+";"+ str(salida3)+";"+str( v_avg_ph_n)+ ";"+str(pfr)+";"+ str(vtn)+ ";" +str(pfs) +\
               ";"+str(i_unbal)+";"+ str(v_avg_ph_ph)+ ";"+ str(pft)+";"+ str(v_unbal)+ ";"+ str(vrn)+";"+str(vsn) + os.linesep
# ELIMINANDO CORCHETE DEL BKP
    ...
    print("DATA GUARDADA EN LOG-BKP-M3.txt..." + str(reg_losti))
# CREANDO CSV Y POBLANDO EL BKP
    data_csv_bkp = open("LOG-BKP-M3.txt")
    s = [reg_losti]
    lineas = data_csv_bkp.readlines()
    create_cvs = csv.writer(open("LOG-BKP-EXPORT-M3.csv", "a"), delimiter=';', quoting=csv.QUOTE_ALL)
    create_cvs.writerow(s)

    print("DATOS ESCRITOS EN LOG-BKP-EXPORT-M3.csv...")